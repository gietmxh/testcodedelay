...
exports.runError = async (arrayStatus) => {
    ...
    for (let data of listError) {
        ...
        let runReward = await this.rewardTransId(null, data.transId);
...
exports.handleTransId = async (data) => {
    try {
        ...
        if (io == -1) comment && comment.includes(dataSetting.withdrawComment) ? status = 'withdraw' : status = 'transfer';
    ...
    try {
        ...
        if (dataHistory.status == 'limitBet' || dataHistory.status == 'errorComment' || status == 'errorComment' || status == 'limitBet') {
            ...
            if (checkLimit) {
                ...
                checkLimit == 1 ? await logHelper.create('rewardTransId', `${phone}|${transId} đã đạt giới hạn, đổi sang số ${phoneNew} để hoàn tiền`) : await logHelper.create('rewardTransId', `${phone}|${transId} không tìm thấy hoặc trạng thái khác, đổi sang số ${phoneNew} để hoàn tiền!`);
        ...
        if (checkJackpot.isJoin == 1) {
            ...
            if (jackpot) {
                await jackpotHelper.rewardJackpot(phone, dataHistory.partnerId, transId, jackpot);
            }
        }
...
exports.handleTransfer = async (phone, transId, message) => {
    try {
         ...
         if (!message) {
            await historyModel.findOneAndUpdate({ transId, io: 1 }, { $set: { status: 'errorPhone', description: 'Chuyển tiền thất bại, undefined'} });
        ...
        if(message.includes('100.000.000đ/tháng')){
