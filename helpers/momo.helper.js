...
exports.randomDevice = () => {
    let listDevice = [
        { "name": "SM-G532F", "deviceOS": "ANDROID", "hardware": "mt6735", "facture": "samsung", "MODELID": "samsung sm-g532gmt6735r58j8671gsw" },
...
exports.diffDays = (date, otherDate) => Math.abs(date - otherDate) / (1000 * 60 * 60 * 24);
...
exports.userCheck = async (phone) => {
    try {
        ...
        if (!response.result) {
            ...
            await momoModel.findOneAndUpdate({ phone }, { $set: { phone, SECUREID, imei, dataDevice, loginStatus: 'waitSend' } }, { upsert: true })
            return ({
                success: true,
                message: 'Success!'
            })
...
exports.login = async (phone) => {
    try {
        ...
        if (!response.result) {
            ...
            await logHelper.create('momoLogin', 'Có lỗi xảy ra ' + response.errorDesc);
            return ({
                success: false,
                message: response.errorDesc
            })
        ...
        await momoModel.findOneAndUpdate({ phone }, { $set: { money: response.extra.BALANCE ?? 0, AUTH_TOKEN, REFRESH_TOKEN, REQUEST_ENCRYPT_KEY, loginAt: new Date(), loginStatus: 'active' } }, { upsert: true })
        ...
        await logHelper.create('momoLogin', 'Có lỗi xảy ra, ' + err.message || err);
        return ({
            success: false,
            message: 'Có lỗi xảy ra ' + err.message || err
        })
...
exports.refreshToken = async (phone) => {
    try {
        ...
        let options = {
            ...
            data: JSON.stringify({
                ...
                momoMsg: {
                    ...
                    refreshToken: dataPhone.REFRESH_TOKEN
                },
                extra: {
                    AAID: dataPhone.AAID,
                    IDFA: "",
                    ...
                    MODELID: dataDevice.MODELID,
                    DEVICE_TOKEN: "",
                    checkSum: this.checkSum(phone, "GENERATE_TOKEN_AUTH_MSG", times, dataPhone.setupKey)
                }
            })
        };
        ...
        await logHelper.create('refreshToken', 'Có lỗi xảy ra ' + err.message || err);
        return ({
            success: false,
            message: 'Có lỗi xảy ra ' + err.message || err
        });
...
exports.getDetails = async (phone, transId) => {
    try {
        ...
        let checkTime = parseInt(this.diffDays(moment(), moment(response.momoMsg.lastUpdate).valueOf()));
    ...
    return new Promise((resolve, reject) => {
        request(options, async (err, response, body) => {
            try {
                ...
                return body.result ? await momoModel.findOneAndUpdate({ phone }, { $set: { money: body.momoMsg.sofInfo[0].balance } }, { upsert: true }) && resolve({
                    success: true,
                    message: 'Lấy thành công!',
...
exports.checkReceiver = async (phone, receiver) => {
    try {
        let times = new Date().getTime();
        const checkSession = await this.checkSession(phone);
        if (!checkSession.success) return checkSession;
...
exports.M2MU_INIT = async (phone, dataTransfer) => {
    try {
        ...
        let checkName = await this.checkReceiver(phone, dataTransfer.phone);
...
exports.moneyTransfer = async (phone, dataTransfer) => {
    try {
        ...
        if (!checkBalance.success) return checkBalance;
        if (checkBalance.balance < dataTransfer.amount) {
            return ({
                success: false,
                message: `Số dư ${phone} không đủ ${dataTransfer.amount}đ để chuyển khoản!`
            })
        }
            ...
            request(options, async (err, response, body) => {
                try {
                    ...
                    if (!body.result) {
                        ...
                        await momoModel.findOneAndUpdate({ phone }, { money: body.extra.BALANCE });
