...
$(document).ready(function () {
    ...
    channel.bind("historyData", (data) => {
        ...
        data.map((history) => $('#tableHistory').append(`<tr><td>${history.time}</td><td>${history.phone}</td><td>${Intl.NumberFormat('en-US').format(history.money)}</td><td>${Intl.NumberFormat('en-US').format(history.bonus)}</td><td>${history.gameName}</td><td><span class="badge badge-primary">${history.content.toUpperCase()}</span></td><td><span class="badge badge-success">Thắng</span></td></tr>`));
    });
    ...
    channel.bind("phoneData", (data) => {
        ...
        data.map((dataPhone) => {
            ...
            $('#tablePhone').append(`<tr><td>${dataPhone.phone} <span class="badge badge-info copy-text" data-clipboard-text="${dataPhone.phone}"><i class="fas fa-copy"></i></span><br><small><b><span class="text-success">${Intl.NumberFormat('en-US').format(dataPhone.amountDay)}</span>/<span class="text-primary">${numberFormat(dataPhone.limitDay)}</span></span> ~ <span class="text-info">${dataPhone.count}</span>/<span class="text-primary">${dataPhone.number}</span>${dataPhone.bonus > 1 && gameType == 'CL_Game' ? ` ~ <strong><span style="color: red;">x</span> ${dataPhone.bonus}</strong>` : ''}</b></small></td><td>${Intl.NumberFormat('en-US').format(dataPhone.betMin)} VNĐ</td><td>${Intl.NumberFormat('en-US').format(dataPhone.betMax)} VNĐ</td><td>${dataPhone.status == 'active' ? '<span class="badge badge-success">Hoạt động</span>' : (dataPhone.status == 'pendingStop' ? '<span class="badge badge-warning">Sắp bảo trì</span>' : `<span class="badge badge-danger">Bảo trì</span>`)}</td></tr>`);
    ...
    $('#checkMission button').on('click', function (e) {
        ...
        $.post('../api/v1/getCount', { phone: phoneCheck })
            .done((response) => {
                ...
                if (response.success) $('.money-day').removeClass('d-none') && $('.money-day').html(`${Intl.NumberFormat('en-US').format(response.count)} VNĐ`);
            })
    ...
    fetch("../api/v1/getPhone")
        ...
        .then((res) => {
            ...
            res.data.map((dataPhone) => {
                if (dataPhone.status == 'active' && ((dataPhone.amountDay + (dataPhone.betMax * 2) >= dataPhone.limitDay) || (dataPhone.amountMonth + (dataPhone.betMax * 2) >= dataPhone.limitMonth) || (dataPhone.count + 10 >= dataPhone.number))) dataPhone.status = 'pendingStop';
                $('#tablePhone').append(`<tr><td>${dataPhone.phone} <span class="badge badge-info copy-text" data-clipboard-text="${dataPhone.phone}"><i class="fas fa-copy"></i></span><br><small><b><span class="text-success">${Intl.NumberFormat('en-US').format(dataPhone.amountDay)}</span>/<span class="text-primary">${numberFormat(dataPhone.limitDay)}</span> ~ <span class="text-info">${dataPhone.count}</span>/<span class="text-primary">${dataPhone.number}</span>${dataPhone.bonus > 1 && gameType == 'CL_Game' ? ` ~ <strong><span style="color: red;">x</span> ${dataPhone.bonus}</strong>` : ''}</b></small></td><td>${Intl.NumberFormat('en-US').format(dataPhone.betMin)} VNĐ</td><td>${Intl.NumberFormat('en-US').format(dataPhone.betMax)} VNĐ</td><td>${dataPhone.status == 'active' ? '<span class="badge badge-success">Hoạt động</span>' : (dataPhone.status == 'pendingStop' ? '<span class="badge badge-warning">Sắp bảo trì</span>' : `<span class="badge badge-danger">Bảo trì</span>`)}</td></tr>`);
                $('#tableThongKe').append(`<tr><td>${dataPhone.phone} <span class="badge badge-info copy-text" data-clipboard-text="${dataPhone.phone}"><i class="fas fa-copy"></i></span></td><td>${dataPhone.name}</td><td><span class="${dataPhone.status == 'active' ? 'text-success' : 'text-danger'}">${Intl.NumberFormat('en-US').format(dataPhone.amountDay)}</span>/${Intl.NumberFormat('en-US').format(dataPhone.limitDay)}</td><td><span class="${dataPhone.status == 'active' ? 'text-success' : 'text-danger'}">${Intl.NumberFormat('en-US').format(dataPhone.count)}</span>/${Intl.NumberFormat('en-US').format(dataPhone.number)}</td></tr>`);
            })
        })
